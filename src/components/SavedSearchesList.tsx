...
const formatCriteria = (search: SavedSearch): string => {
  ...
  if (search.bathrooms !== 'any') parts.push(`${search.bathrooms} bath${search.bathrooms !== '1' ? 's' : ''}`);
    ...
    <div className="bg-white p-6 rounded-lg shadow-md mb-8">
      ...
      <ul className="space-y-3">
        {searches.map(search => (
          <li 
            key={search.id}
            className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border border-slate-200 rounded-md bg-slate-50"
          >
